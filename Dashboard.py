...
#region [ 5. IP 성과 자세히보기 (안전판) ]
# =====================================================================
# - NameError 방지: 의존 함수/변수( pd, fmt_eokman, _pct_color, sublines_html, kpi_with_rank )를 함께 정의
# - 데이터 없거나 컬럼 누락이면 st.stop() 사용하지 않고 경고 후 return
# - 디지털 조회수는 #억####만(만 이하 절삭)으로 표기
# - 보조 문구는 '그룹 내 N위' / '그룹 평균比 P%' 고정
# - 타이틀은 .page-title 클래스로 박스 해제 + 사이즈 복구
# =====================================================================
import streamlit as st
import pandas as pd
import numpy as np
...
# ---- 공통 헬퍼: 세션/글로벌에서 원본 DF 찾기
def _pick_df_for_detail():
    cand_keys = [
        'DF_DETAIL','DF_IP','DF_RAW','DF_MASTER','DF',
        'df_detail','df_ip','df_raw','df_master','df',
        'RAW','RAW_원본','RAW_MASTER','DATA','data'
    ]
    for k in cand_keys:
        if k in st.session_state and isinstance(st.session_state[k], pd.DataFrame):
            return st.session_state[k]
    for k in cand_keys:
        if k in globals() and isinstance(globals()[k], pd.DataFrame):
            return globals()[k]
    return None

# ---- 공통 헬퍼: 퍼센트 색상(상대 비교)
def _pct_color(val, base):
    try:
        if base in (None, 0) or pd.isna(base) or pd.isna(val):
            return "#888"
        ratio = float(val) / float(base)
        if ratio > 1.05:   # +5% 이상
            return "#0057E7"  # 블루 톤(상승)
        if ratio < 0.95:   # -5% 이하
            return "#D64541"  # 레드 톤(하락)
        return "#444"      # 보합
    except Exception:
        return "#888"
...
# ---- 디지털 조회수: #억####만 (만 이하 절삭)
def fmt_eokman(n):
    if n is None or (hasattr(pd, "isna") and pd.isna(n)):
        return "–"
    try:
        n = int(float(n))
    except Exception:
...
# ---- KPI 렌더러(디지털 조회수는 fmt_eokman 강제)
def kpi_with_rank(col, title, value, base_val, rank_tuple, prog_label, intlike=False, digits=3):
    with col:
        if value is None or (hasattr(pd, "isna") and pd.isna(value)):
            main = "–"
        else:
            if '디지털 조회' in title or '디지털 조회수' in title:
                main = fmt_eokman(value)
            else:
                main = f"{int(value):,}" if intlike else f"{float(value):,.{digits}f}"
        st.markdown(
...
def render_ip_detail():
    ...
    df = _pick_df_for_detail()
    if df is None or not isinstance(df, pd.DataFrame) or df.empty:
        st.warning("표시할 데이터가 없습니다. (원본 DF를 찾지 못했거나 비어 있음)")
        return

    # 최소 컬럼 방어: IP, metric/value/매체/편성 등이 있을 수도 있고 없을 수도 있음
    # 디지털 조회 집계에 우선 필요한 컬럼을 확인
    # 예상되는 디지털 조회 metric 라벨 후보
    dig_keywords = ['디지털 조회', '디지털 조회수', '디지털_조회', 'Digital Views', 'digital_views']

    # IP 선택
    ip_col = 'IP' if 'IP' in df.columns else ('작품' if '작품' in df.columns else None)
    if ip_col is None:
        st.info("IP 컬럼을 찾지 못했습니다. (IP / 작품 중 하나가 필요)")
        return

    ips = sorted([str(x) for x in df[ip_col].dropna().unique().tolist()])
    if not ips:
        st.info("선택 가능한 IP가 없습니다.")
        return
    sel_ip = st.selectbox("IP 선택", ips, index=0, key="ip_detail_sel_ip")

    # 그룹(비교 집단) 정의: 동일 편성/연도 등이 있으면 필터; 없으면 전체
    base_df = df.copy()
    prog_label = sel_ip  # KPI 보조에 표시할 라벨

    # 디지털 조회수 계산(선택 IP vs 그룹 평균/순위)
    # metric / 매체 식별
    metric_col = None
    for c in ['metric', '지표', '지표구분', 'METRIC']:
        if c in df.columns:
            metric_col = c
            break

    value_col = None
    for c in ['value', '값', 'VALUE', 'val']:
        if c in df.columns:
            value_col = c
            break

    if value_col is None:
        st.info("값 컬럼(value)이 필요합니다.")
        return

    # 디지털 조회 필터
    if metric_col:
        is_digital = df[metric_col].astype(str).str.contains('|'.join(dig_keywords), case=False, na=False)
        df_dig = df[is_digital].copy()
    else:
        # metric이 없으면 매체/컬럼명에서 추정 실패 → 전체에서 쓰되 경고
        st.caption("참고: metric 컬럼이 없어 디지털 조회 추정을 생략하고 전체 값으로 계산합니다.")
        df_dig = df.copy()
    ...
    g_all = df_dig.groupby(ip_col, as_index=False)[value_col].apply(_safe_sum).rename(columns={value_col: 'digital_sum'})
    # 선택 IP 값
    sel_val = g_all.loc[g_all[ip_col] == sel_ip, 'digital_sum']
    sel_val = float(sel_val.iloc[0]) if not sel_val.empty else np.nan
    # 그룹 평균
    base_val = float(g_all['digital_sum'].mean()) if not g_all.empty else np.nan
    # 순위(내림차순: 값 클수록 1위)
    g_all = g_all.sort_values('digital_sum', ascending=False).reset_index(drop=True)
    if not np.isnan(sel_val):
        rnk = int(g_all.index[g_all[ip_col] == sel_ip][0]) + 1
        total = int(len(g_all))
        rank_tuple = (rnk, total)
    else:
        rank_tuple = (None, len(g_all))

    # KPI 렌더(디지털 조회는 #억####만)
    c1, c2, c3 = st.columns([1, 1, 1])
    kpi_with_rank(c1, "디지털 조회수", sel_val, base_val, rank_tuple, prog_label, intlike=True)
    # 예시로 보조 KPI 2개 더(원하면 실제 지표로 대체)
    # 평균 대비 비율 / 순위만 요약 텍스트로
    ratio_txt = "–"
    try:
        ratio_txt = f"{(sel_val/base_val)*100:.0f}%" if (base_val not in (None, 0) and not np.isnan(base_val)) else "–"
    except Exception:
        ratio_txt = "–"
    c2.metric("그룹 평균比", ratio_txt)
    c3.metric("그룹 내 순위", f"{rank_tuple[0]}/{rank_tuple[1]}" if rank_tuple[0] else "–")

    # (옵션) 간단 추이 차트: 회차 혹은 주차 기준 디지털 조회
    # 회차/주차 컬럼 후보
    time_col = None
    for c in ['회차', 'ep', 'episode', 'EP', '주차시작일', '주차', 'week_start']:
        if c in df_dig.columns:
            time_col = c
            break

    if time_col:
        _sel = df_dig[df_dig[ip_col] == sel_ip].copy()
        if not _sel.empty:
            _sel[time_col] = pd.to_datetime(_sel[time_col], errors='ignore')
            _g = _sel.groupby(time_col, as_index=False)[value_col].apply(_safe_sum).rename(columns={value_col: 'value'})
            fig = px.line(_g, x=time_col, y='value', markers=True, title=None)
            fig.update_traces(hovertemplate="%{x}<br>%{y:,}")
            fig.update_layout(margin=dict(l=6, r=6, t=6, b=6), height=360, xaxis_title=None, yaxis_title=None)
            st.plotly_chart(fig, use_container_width=True, theme=None)
        else:
            st.info("선택 IP의 추이 데이터가 없습니다.")
    else:
        st.caption("참고: 회차/주차 컬럼이 없어 추이 그래프는 생략합니다.")
